...
// New action to sync emails to database
// Action to modify Gmail message labels (for two-way sync)
export const modifyGmailLabels = action({
  args: {
    userId: v.optional(v.id("users")), // Optional userId for scheduled actions
    gmailId: v.string(),
    addLabelIds: v.optional(v.array(v.string())),
    removeLabelIds: v.optional(v.array(v.string())),
  },
  handler: async (ctx, args): Promise<{ success: boolean; error?: string; data?: any }> => {
    // If userId is provided (from scheduled action), use it directly
    // Otherwise, get from auth context (for direct calls)
    const user = args.userId 
      ? await ctx.runQuery(api.users.getUserById, { userId: args.userId })
      : await ctx.runQuery(api.gmail.getCurrentUserWithTokens);
    ...
    if (!user.googleAccessToken) {
      console.error("No Google access token found for label modification");
      return { success: false, error: "No Gmail access" };
    }
    ...
    try {
      ...
      const response = await fetch(
        `https://gmail.googleapis.com/gmail/v1/users/me/messages/${args.gmailId}/modify`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
      ...
      if (!response.ok) {
        ...
        console.error(`Failed to modify Gmail labels: ${response.status} - ${errorText}`);
        return { success: false, error: `Failed to modify labels: ${response.statusText}` };
      }

      const result = await response.json();
      console.log(`Successfully modified labels for message ${args.gmailId}`);
      
      return { success: true, data: result };
    } catch (error) {
      console.error("Error modifying Gmail labels:", error);
      if (error instanceof ConvexError) {
        throw error;
      }
      return { success: false, error: "Failed to modify Gmail labels" };
    }
  },
});
